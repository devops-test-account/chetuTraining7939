                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-primary">
                        <div class="card-header">
                            <h3 class="card-title">Update <small>{{$data['name']}}</small></h3>
                        </div>
                        <!-- /.card-header -->
                        <!-- form start -->
                        <form action="{{route('tasks.update',$data['id'])}}" method="post" id="quickForm">
                            @method('PUT')
                            @csrf
                            <div class="card-body">
                                <div class="form-group">
                                    <label for="exampleInputName">Name</label>
                                    <input type="text" name="name" class="form-control" id="exampleInputName" value="{{$data['name']}}" placeholder="Enter Name">
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputDescription">Description</label>
                                    <textarea name="description" class="form-control" id="exampleInputDescription">{{$data['description']}}</textarea>
                                </div>
                                <div class="form-group">
                                    ...
                                    <select name="assigned_to" id="assigned_to" class="form-control select2bs4">
                                        ...
                                        <option value="{{$userID}}" {{$data['assigned_to'] === $userID ? "selected" : ""}}>{{$user[0]['name']}}</option>
